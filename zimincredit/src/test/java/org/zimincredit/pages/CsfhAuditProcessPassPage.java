...
public class CsfhAuditProcessPassPage extends CommonPage{
	...
	public void clickSubmitAuditButton(String productName){
		...
		if(TPages <= 0){
			...
			for(int i = 0;i < pageProductNumber.size();i++){
				if(productName.equals(pageProductNameList.get(i).getText()) && "待复核".equals(pageProductStatusList.get(i).getText())){
					int k = i + 1;
			...
			for(int i = 1;i <= TPages;){
				for(int j = 0;j < pageProductNameList.size();j++){
					if(productName.equals(pageProductNameList.get(j).getText()) && "待复核".equals(pageProductStatusList.get(j).getText())){
						int k = j + 1;
						String xpath = "//tbody[@id='app_list']/tr[" + k + "]/td[9]/a";
						List<WebElement> operateButtons = driver.findElements(By.xpath(xpath));
						click(operateButtons.get(1));
	...
	public void checkProductStatusChanged(){
		if(qureyproductresult == true && productStatus.getText().equals("已完成")){
			System.out.println("--------> 初审复核人员提交复核成功！");
		}
	}
}
