...
public class KfzyAuditProcessPassPage extends CommonPage{
...
//	@FindBy(xpath = "//tbody[@id='app_list']/tr/td[9]/a")
//	private List<WebElement> operateButtons;
	
	private WebElement productStatus = null;
	private boolean qureyproductresult = false;

	
	public void clickSubmitauditbutton(String productName){
		int TPages = totalSpan.size() - 2;
		if(TPages <= 0){
			System.out.println(productName + "没有查询到产品！");
		}else if (TPages == 1){
			for(int i = 0;i < pageProductNumber.size();i++){
				if(productName.equals(pageProductNameList.get(i).getText()) && "待提交".equals(pageProductStatusList.get(i).getText())){
					int k = i + 1;
...
//					action.keyUp(Keys.ENTER);
			...
			for(int i = 1;i <= TPages;){
				for(int j = 0;j < pageProductNameList.size();j++){
					if(productName.equals(pageProductNameList.get(j).getText()) && "待提交".equals(pageProductStatusList.get(j).getText())){
						int k = j + 1;
						String xpath = "//tbody[@id='app_list']/tr[" + k + "]/td[9]/a";
						List<WebElement> operateButtons = driver.findElements(By.xpath(xpath));
						int buttonNumber = operateButtons.size() - 1;
