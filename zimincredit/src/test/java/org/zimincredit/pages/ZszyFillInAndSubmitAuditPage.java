...
public class ZszyFillInAndSubmitAuditPage extends CommonPage{
	...
	@FindBy(xpath = "//div[@class='form-inline clearfix']/div[2]/div[1]/div/span/span/select")
	private WebElement approvalResultX;
	
	@FindBy(xpath = "//div[@class='form-inline clearfix']/div[3]/div[1]/div/span/span/select")
	...
	public void fillInProductField(DataTable table){
		if(queryproductresult == true){
			...
			String validationResult = data.get(2).get(1);
			String approvalReason = data.get(3).get(1);
			Select approvalResultSelect = new Select(approvalResultX);
			approvalResultSelect.selectByVisibleText(approvalResult);
			Select validationResultSelect = new Select(validationResultX);
			validationResultSelect.selectByVisibleText(validationResult);
	...
	public void staffAllocationTo(){
		if(queryproductresult == true){
			...
			assignUser.selectByVisibleText("终审复核01");
