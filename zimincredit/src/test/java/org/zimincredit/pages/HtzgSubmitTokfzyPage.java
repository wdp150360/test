...
public class HtzgSubmitTokfzyPage extends CommonPage{
	...
	public void clickSubmitButton(String productName){
		int TPages = totalSpan.size() - 2;
		if(TPages <= 0){
			System.out.println(productName + "没有查询到产品！");
		}else if (TPages == 1){
			for(int i = 0;i < pageProductNumber.size();i++){
				if(productName.equals(pageProductNameList.get(i).getText()) && "已处理-待签约".equals(pageProductStatusList.get(i).getText())){
					int k = i + 1;
			...
			for(int i = 1;i <= TPages;){
				for(int j = 0;j < pageProductNameList.size();j++){
					if(productName.equals(pageProductNameList.get(j).getText()) && "已处理-待签约".equals(pageProductStatusList.get(j).getText())){
						int k = j + 1;
						String xpath = "//tbody[@id='app_list']/tr[" + k + "]/td[9]/a";
						List<WebElement> operateButtons = driver.findElements(By.xpath(xpath));
						click(operateButtons.get(1));
	...
	public void checkProductStatusChanged(){
		if(qureyproductresult == true && productStatus.getText().equals("已完成")){
			System.out.println("--------> 合同主管提交进件到客服专员上传合同成功！");
		}
	}
}
